...
import { 
  ...
  Home, X, AlertTriangle, CheckCircle2, TrendingDown, BookOpen
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
// @ts-ignore
import confetti from "canvas-confetti";

// --- UPDATED ACADEMIC RANKS (Muted & Professional) ---
const RANKS = [
  { name: "Novice", minXp: 0, color: "#94a3b8" },      // Slate
  { name: "Apprentice", minXp: 500, color: "#64748b" }, // Steel
  { name: "Scholar", minXp: 1500, color: "#475569" },   // Deep Slate
  { name: "Analyst", minXp: 3000, color: "#818cf8" },   // Soft Indigo
  { name: "Specialist", minXp: 5000, color: "#6366f1" },// Indigo
  { name: "Expert", minXp: 8000, color: "#4f46e5" },    // Royal Indigo
  { name: "Master", minXp: 12000, color: "#4338ca" },   // Deep Indigo
  { name: "Doctorate", minXp: 18000, color: "#3730a3" },// Dark Indigo
  { name: "Professor", minXp: 25000, color: "#1e1b4b" } // Midnight
];
...
export default function AssessmentCentre() {
  ...
  const [rankLeaderboard, setRankLeaderboard] = useState<any[]>([]);
  ...
  // --- REFINED THEME COLOR ---
  const themeColor = "rgba(99, 102, 241, 0.08)"; // Soft Indigo instead of Blue
  ...
  useEffect(() => {
    ...
    async function initArena() {
      ...
      try {
        ...
        const [subRes, profRes, perfRes] = await Promise.all([
        ...
        if (profRes?.data) {
          ...
          const { current, next } = getRankDetails(profile.xp || 0);
          let query = supabase.from('profiles').select('id, username, xp').gte('xp', current.minXp).order('xp', { ascending: false }).limit(5);
          if (next) query = query.lt('xp', next.minXp);
          const { data: rankLeaderboardData } = await query;
          if (rankLeaderboardData) setRankLeaderboard(rankLeaderboardData);
        }
  ...
  if (checkingAuth) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Verifying Credentials...</p>
      </div>
    );
  }
  ...
  const fetchSubjectMasters = async (subjectId: string) => {
    ...
    if (data) {
      ...
      data.forEach((entry: any) => {
        const username = entry.profiles?.username || "Student";
  ...
  const resetQuizState = () => {
    setCurrentQIndex(0); setScore(0); setStreak(0); setMaxStreak(0); setTimeInSeconds(0);
    setSelectedOptions([]); setIsAnswerChecked(false); setSessionHistory([]);
    setShouldShake(false); hasSubmitted.current = false; setShowExitModal(false);
  };

  const returnToModuleSelection = () => {
    resetQuizState(); setSelectedChapters([]);
    if (selectedSubject) fetchSubjectMasters(selectedSubject.id);
    setView('selection'); setStep(2); 
  };

  const handleSubjectSelect = async (sub: any) => {
    setSelectedSubject(sub); setLoading(true);
    const { data: allQ } = await supabase.from('questions').select('chapter_id');
    ...
    setModules(modData.data || []); setStep(2); setLoading(false);
  };
  ...
  const handleModuleSelect = async (mod: any) => {
    ...
    const { data: chapData } = await supabase.from('chapters').select('*').eq('subject_key', mod.slug).in('id', activeIds).order('created_at');
    if (chapData) {
      setChapters(chapData);
      const counts: Record<string, number> = {};
      qData?.forEach((q: any) => { if(chapData.some(c => c.id === q.chapter_id)) counts[q.chapter_id] = (counts[q.chapter_id] || 0) + 1; });
  ...
  const startQuiz = async (mastery = false) => {
    setLoading(true); setIsMasteryExam(mastery);
  ...
  const checkAnswer = () => {
    ...
    const isCorrect = selectedOptions.length === currentQ.correct_indices.length && selectedOptions.every(i => currentQ.correct_indices.includes(i));
    setSessionHistory(prev => [...prev, { question: currentQ.question_text, options: currentQ.options, selected: [...selectedOptions], correct: currentQ.correct_indices, isCorrect }]);
    ...
    if (isCorrect) {
      ...
      setStreak(0); setShouldShake(true); setTimeout(() => setShouldShake(false), 400);
    }
  };
  ...
  const handleFinishQuiz = async () => {
    ...
    const xp = score * (isMasteryExam ? 20 : 10);
    setXpEarned(xp);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        await supabase.rpc('increment_user_xp', { user_id: user.id, xp_to_add: xp });
        await supabase.from('quiz_performances').insert([{ user_id: user.id, subject_id: selectedSubject.id, score, total_questions: questions.length, time_seconds: timeInSeconds, is_mastery: isMasteryExam }]);
  ...
  return (
    <main className="relative overflow-x-hidden w-full min-h-[100dvh] text-white bg-[#050505] flex flex-col font-sans selection:bg-indigo-500/30">
        ...
        {showExitModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} className="bg-[#0a0a0a] border border-white/10 p-8 rounded-[2rem] max-w-sm w-full text-center shadow-2xl">
              ...
              <h3 className="text-xl font-black uppercase italic mb-2 tracking-tighter">Abort Session?</h3>
              <p className="text-white/40 font-bold text-[10px] mb-8 uppercase tracking-widest">Unsaved progress will be discarded.</p>
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => setShowExitModal(false)} className="w-full py-4 bg-white/5 border border-white/10 rounded-xl font-black uppercase text-[10px] hover:bg-white/10 transition-colors">Resume</button>
              </div>
            </motion.div>
          </motion.div>
        )}
          ...
          {view === 'lobby' && (
            <motion.div key="lobby" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-6 md:gap-10 h-full justify-center min-h-[80vh]">
              <header className="relative text-center">
                <button onClick={() => router.push('/')} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 p-4 rounded-full bg-white/5 hover:bg-white/10 transition-colors z-50"><Home size={24} /></button>
                <h1 className="text-4xl md:text-7xl lg:text-8xl font-black italic uppercase tracking-tighter leading-none drop-shadow-2xl">Assessment <span className="text-indigo-500">Centre</span></h1>
              </header>
          
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="flex flex-col gap-6 order-1 lg:order-2">
                  <div className="bg-white/[0.03] border border-white/10 p-6 rounded-[2rem] backdrop-blur-xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity"><Crown size={48} /></div>
                    <div className="mb-8 border-b border-white/5 pb-8">
                        <div className="flex justify-between items-end mb-2">
                           <p className="text-[10px] font-black uppercase opacity-40 tracking-widest">Academic Status</p>
                           <span className="text-xs font-black" style={{ color: rankInfo.current.color }}>{rankInfo.current.name}</span>
                        </div>
                        <h4 className="text-3xl font-black mb-4 text-white">
   {userProfile?.username || "Student"}
</h4>
                        <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden mb-2">
                            <motion.div initial={{ width: 0 }} animate={{ width: `${rankInfo.progress}%` }} className="h-full shadow-[0_0_15px_rgba(255,255,255,0.2)]" style={{ backgroundColor: rankInfo.current.color }} transition={{ duration: 1.5, ease: "easeOut" }} />
                        </div>
                        <div className="flex justify-between text-[9px] font-bold uppercase opacity-50 tracking-wider">
                            <span>{userProfile?.xp || 0} Credits</span>
                            <span>{rankInfo.next ? `${rankInfo.next.name}` : 'Max'}</span>
                        </div>
                    </div>
                    <div>
                        <div className="text-amber-500 font-black text-[10px] uppercase mb-4 flex items-center gap-2 tracking-widest"><Trophy size={14}/> Top Performers</div>
                        <div className="space-y-3">
                          {rankLeaderboard.length > 0 ? rankLeaderboard.map((u, i) => (
                              <div key={i} className={`flex justify-between items-center py-2 px-3 rounded-xl ${u.id === userProfile?.id ? 'bg-white/10 border border-white/10' : 'hover:bg-white/5'} transition-colors`}>
                                 <div className="flex items-center gap-3">
                                    <span className={`text-[10px] font-black w-4 ${i===0 ? 'text-amber-500' : 'opacity-30'}`}>#{i+1}</span>
                                    <span className="text-xs font-bold opacity-90">{u.username}</span>
                                 </div>
                                 <span className="text-[10px] font-black opacity-60">{u.xp} CR</span>
                              </div>
                           )) : <div className="text-center py-4 opacity-30 text-[10px] uppercase font-bold">No Records Found</div>}
                        </div>
                    </div>
                  </div>
                  {weakSubjects.length > 0 && (
                    <div className="bg-red-500/5 border border-red-500/10 rounded-[2rem] p-6 backdrop-blur-3xl">
                      <div className="text-red-500 font-black text-[10px] uppercase mb-4 flex items-center gap-2 tracking-widest"><TrendingDown size={14}/> Focus Areas</div>
                      <div className="space-y-3">
                ...
                {/* --- UPDATED ACTION CARD (Professional Gradient) --- */}
                <div onClick={() => { setView('selection'); setStep(1); }} className="lg:col-span-2 bg-gradient-to-br from-slate-900 to-indigo-950 border border-indigo-500/20 rounded-[2rem] p-8 md:p-12 flex flex-col items-center justify-center cursor-pointer hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 group relative overflow-hidden shadow-2xl shadow-indigo-900/20 min-h-[250px] md:min-h-[350px] order-2 lg:order-1">
                  <BrainCircuit className="absolute -right-12 -bottom-12 opacity-5 group-hover:rotate-12 transition-transform duration-1000 w-[200px] h-[200px] md:w-[350px] md:h-[350px] text-indigo-400" />
                  <h2 className="text-3xl md:text-5xl lg:text-6xl font-black italic uppercase mb-8 z-10 text-center leading-tight">Initialize Assessment</h2>
                  <button className="px-10 md:px-14 py-4 md:py-5 bg-white text-black rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-[0.2em] z-10 flex items-center gap-3 shadow-xl group-hover:bg-indigo-50 transition-colors">
                      <Play size={18} fill="currentColor"/> Start Evaluation
                  </button>
                </div>
              </div>
            </motion.div>
          ...
          {view === 'selection' && (
            <motion.div key="selection" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full flex-1 flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <button onClick={() => step === 1 ? setView('lobby') : setStep(step - 1)} className="flex items-center gap-2 text-[10px] font-black uppercase opacity-40 hover:opacity-100 transition-opacity"><ArrowLeft size={14}/> Back</button>
              </div>
              {loading ? (
                  ...
                  {step === 1 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                      {rootSubjects.map(sub => (
                        <div key={sub.id} onClick={() => handleSubjectSelect(sub)} className="group p-8 md:p-12 rounded-[2.5rem] bg-white/[0.03] border border-white/10 hover:border-indigo-500/30 hover:bg-white/[0.05] cursor-pointer transition-all active:scale-[0.97]">
                  ...
                  {step === 2 && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                      <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6 h-fit order-1 lg:order-2">
                        <div className="text-emerald-500 font-black text-[10px] uppercase mb-4 flex items-center gap-2 tracking-widest"><Trophy size={14}/> Academic Leaders</div>
                          ...
                          {subjectMasters.length > 0 ? subjectMasters.map((m, i) => (
                            <div key={i} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
                              <div className="flex flex-col">
                                 ...
                                 <span className="text-[8px] font-black text-emerald-500 uppercase tracking-tighter mt-0.5">HONORS</span>
                              </div>
                              <span className="text-[10px] font-black opacity-40 font-mono">{formatTime(m.time_seconds)}</span>
                            </div>
                          )) : <div className="py-8 text-center opacity-30"><Trophy size={24} className="mx-auto mb-2"/><p className="text-[10px] uppercase font-bold">No Records</p></div>}
                        </div>
                      </div>
                      ...
                      <div className="lg:col-span-2 space-y-4 order-2 lg:order-1">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-white/[0.05] to-transparent p-6 rounded-[2rem] border border-white/5">
                            ...
                            <button onClick={() => startQuiz(true)} className="w-full md:w-auto px-6 py-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center gap-2">
                                <Crown size={14}/> Proficiency Exam
                            </button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {modules.map(mod => (
                            <div key={mod.id} onClick={() => handleModuleSelect(mod)} className="p-5 md:p-6 rounded-[1.5rem] bg-white/[0.03] border border-white/10 hover:bg-white/[0.07] hover:border-white/20 cursor-pointer flex justify-between items-center group transition-all">
                              ...
                              <ChevronRight size={18} className="opacity-20 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-indigo-400" />
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  {step === 3 && (
                    <div className="space-y-6">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                        <div>
                            <h1 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter">Target <span className="text-indigo-500">Chapters</span></h1>
                            <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 mt-2">Select one or multiple</p>
                        </div>
                        <button onClick={() => startQuiz(false)} disabled={selectedChapters.length === 0} className="w-full md:w-auto px-10 py-4 bg-white text-black rounded-xl font-black uppercase text-[10px] tracking-widest disabled:opacity-20 disabled:scale-100 active:scale-95 transition-all shadow-xl hover:scale-105 hover:bg-indigo-50">
                          <Flame size={16} className={selectedChapters.length > 0 ? "text-indigo-600 fill-indigo-600" : ""}/> Start Evaluation
                        </button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pb-20 md:pb-0">
                        {chapters.map(chap => (
                          <div key={chap.id} onClick={() => setSelectedChapters(p => p.includes(chap.id) ? p.filter(c => c !== chap.id) : [...p, chap.id])} className={`p-6 rounded-[1.5rem] border transition-all cursor-pointer relative overflow-hidden ${selectedChapters.includes(chap.id) ? 'border-indigo-500 bg-indigo-500/10 shadow-[0_0_30px_rgba(99,102,241,0.1)]' : 'bg-white/[0.03] border-white/10 hover:bg-white/5'}`}>
                            {selectedChapters.includes(chap.id) && <div className="absolute top-0 right-0 p-3"><CheckCircle2 size={16} className="text-indigo-500 fill-indigo-500/20"/></div>}
          ...
          {view === 'quiz' && (
            <motion.div key="quiz" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full relative">
              <div className="fixed top-0 left-0 w-full h-1.5 bg-white/5 z-[200]">
                {/* --- INDIGO PROGRESS BAR --- */}
                <motion.div initial={{ width: 0 }} animate={{ width: `${((currentQIndex + 1) / questions.length) * 100}%` }} className="h-full bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)]" transition={{ type: 'spring', stiffness: 50 }} />
              </div>
              <div className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 bg-[#050505]/90 backdrop-blur-xl border border-white/10 px-6 py-2 rounded-full flex items-center gap-6 z-[100] shadow-2xl scale-90 md:scale-100">
                ...
                <span className="text-indigo-400 font-black text-[10px] uppercase tracking-[0.2em] whitespace-nowrap opacity-60 pl-2 border-l border-white/10">Q.{currentQIndex + 1}/{questions.length}</span>
              </div>
              ...
              <motion.div animate={shouldShake ? { x: [-10, 10, -10, 10, 0] } : {}} className={`flex flex-col justify-center items-center gap-8 w-full max-w-4xl mx-auto flex-1 mt-20 md:mt-0 pb-28 md:pb-0`}>
                ...
                <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                    {questions[currentQIndex]?.options.map((opt: string, idx: number) => {
                      ...
                      // --- UPDATED OPTION STYLES (Indigo Focus) ---
                      let style = isSelected ? "border-indigo-500 bg-indigo-500/10 text-white shadow-[0_0_15px_rgba(99,102,241,0.1)]" : "border-white/10 bg-white/[0.02] hover:bg-white/[0.05]";
                      if (isAnswerChecked) {
                ...
                <div className="fixed bottom-0 left-0 w-full p-4 bg-[#050505]/80 backdrop-blur-xl border-t border-white/10 md:static md:bg-transparent md:border-0 md:p-0 flex justify-end z-[50]">
                  {!isAnswerChecked ?
                    <button disabled={selectedOptions.length === 0} onClick={checkAnswer} className="w-full md:w-auto px-12 py-4 bg-white text-black rounded-xl font-black uppercase text-xs tracking-[0.2em] hover:bg-indigo-500 hover:text-white transition-all shadow-xl disabled:opacity-20 active:scale-95">Commit Choice</button> :
                    <button onClick={() => currentQIndex < questions.length - 1 ? (setCurrentQIndex(currentQIndex + 1), setSelectedOptions([]), setIsAnswerChecked(false)) : handleFinishQuiz()} className="w-full md:w-auto px-12 py-4 bg-indigo-600 text-white rounded-xl font-black uppercase text-xs tracking-[0.2em] flex items-center justify-center gap-4 shadow-lg active:scale-95 transition-all">Next Question <ChevronRight size={16}/></button>
                  }
                </div>
              </motion.div>
            </motion.div>
          ...
          {view === 'results' && (
            <motion.div key="results" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-4xl mx-auto flex-1 w-full pt-8">
              <div className="text-center mb-8 md:mb-12">
                <h1 className="text-5xl md:text-8xl font-black italic uppercase text-white mb-6 tracking-tighter leading-none">Evaluation Concluded</h1>
                <div className="flex flex-col items-center gap-6">
                  <div className="flex justify-center gap-3">
                    <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-emerald-400 font-black uppercase text-[10px] md:text-xs tracking-wider"><Zap size={14}/> +{xpEarned} Credits</div>
                    {maxStreak >= 3 && (<div className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500/10 border border-orange-500/20 rounded-full text-orange-500 font-black uppercase text-[10px] md:text-xs tracking-wider"><Flame size={14}/> {maxStreak} Momentum</div>)}
                  </div>
                  <div className="w-full max-w-sm bg-white/5 p-6 rounded-3xl border border-white/10 backdrop-blur-md">
                      ...
                      <div className="w-full h-3 bg-black/40 rounded-full overflow-hidden border border-white/5">
                        <motion.div 
                          initial={{ width: rankInfo.next ? `${((userProfile?.xp - xpEarned - rankInfo.current.minXp) / (rankInfo.next.minXp - rankInfo.current.minXp)) * 100}%` : '100%' }} 
                          animate={{ width: `${rankInfo.progress}%` }} 
                          className="h-full relative shadow-[0_0_20px_rgba(255,255,255,0.1)]" 
                          style={{ backgroundColor: rankInfo.current.color }} 
                      ...
                      <p className="text-[9px] font-bold uppercase opacity-30 mt-3 tracking-widest">
                        {rankInfo.next ? `${rankInfo.next.minXp - (userProfile?.xp || 0)} Credits to ${rankInfo.next.name}` : "Maximum Rank Achieved"}
                      </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 mb-12">
                <h3 className="text-lg font-black italic uppercase text-white/40 tracking-tighter flex items-center gap-2 px-4"><ShieldCheck size={18}/> Result Analysis</h3>
              ...
              <div className="text-center pb-12">
                <button onClick={returnToModuleSelection} className="w-full md:w-auto px-16 py-6 bg-white text-black rounded-full font-black uppercase text-xs tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 transition-all">Return to Dashboard</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
