...
import { 
  ...
  FileBadge, Archive, CalendarRange, Trash
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch Related Data (Pointing to 'library' table)
      const [chapRes, libRes, quesRes] = await Promise.all([
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-black"><Loader2 className="animate-spin text-purple-500" size={50} /></div>;
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-slate-200">
      ...
      <aside className="w-[420px] bg-[#0a0a0a] border-r border-white/5 p-8 sticky top-0 h-screen overflow-y-auto scrollbar-hide">
        ...
        {/* Semester Toggle */}
        <div className="bg-white/5 p-2 rounded-[1.5rem] flex border border-white/5 mb-8">
          {[1, 2].map((sem) => (
            <button key={sem} onClick={() => setActiveSemester(sem as 1 | 2)}
              className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSemester === sem ? 'bg-purple-600 text-white shadow-lg' : 'text-white/20'}`}>
              Semester {sem}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-8 pb-20">
          {/* Library Deposit - Pointing to 'library' table */}
          <section className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5 relative overflow-hidden">
             <div className="absolute top-0 right-0 p-4 opacity-10 font-black text-6xl text-blue-500">S{activeSemester}</div>
            ...
            <form className="space-y-3" onSubmit={async (e) => {
              e.preventDefault();
              await supabase.from('library').insert([{ ...newLibraryItem, semester: activeSemester }]);
              setNewLibraryItem({ ...newLibraryItem, label: '', file_url: '' });
              fetchData();
            }}>
              <select required className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-[10px] font-bold text-white"
              ...
              <select className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-[10px] font-bold text-white"
              ...
              <input placeholder="Item Label (e.g. Midterm 2024)" className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-xs"
                value={newLibraryItem.label} onChange={e => setNewLibraryItem({...newLibraryItem, label: e.target.value})} required />
              <input placeholder="Drive URL" className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-[10px] font-mono"
          ...
          <section className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5 relative overflow-hidden">
            ...
            <form className="space-y-3" onSubmit={async (e) => { 
                e.preventDefault();
                await supabase.from('subjects').insert([{...newSub, semester: activeSemester, parent_slug: newSub.parent_slug || null}]);
                setNewSub({ slug: '', title: '', icon: '', parent_slug: '' }); 
                fetchData();
            }}>
              <input placeholder="Slug (cytologie)" className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-xs" value={newSub.slug} onChange={e => setNewSub({...newSub, slug: e.target.value})} required />
              <input placeholder="Full Title" className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-xs" value={newSub.title} onChange={e => setNewSub({...newSub, title: e.target.value})} required />
              ...
              <button className="w-full py-4 bg-purple-600 hover:bg-purple-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">Create Module (S{activeSemester})</button>
            </form>
          </section>
          ...
          <section className="bg-white/[0.02] p-6 rounded-[2rem] border border-white/5">
            ...
            <form className="space-y-3" onSubmit={async (e) => { 
              ...
              <select required className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-[10px] font-bold text-white" value={newChap.subject_key} onChange={e => setNewChap({...newChap, subject_key: e.target.value})}>
                <option value="">Link to Subject</option>
                {subjects.filter(s => s.parent_slug).map(s => <option key={s.id} value={s.slug}>{s.title}</option>)}
              </select>
              ...
              <input placeholder="File Link" className="w-full p-4 bg-black/40 border border-white/10 rounded-2xl text-[9px] font-mono" value={newChap.file_url} onChange={e => setNewChap({...newChap, file_url: e.target.value})} />
      ...
      <main className="flex-1 p-16 overflow-y-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'subjects' && (
            <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="space-y-6">
              {subjects.filter(s => !s.parent_slug).length === 0 ? (
                 <div className="p-10 text-center text-white/20 font-black uppercase text-sm tracking-widest">Empty Workspace S{activeSemester}</div>
              ) : (
                subjects.filter(s => !s.parent_slug).map(root => (
                  <div key={root.id} className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] overflow-hidden group">
                    <div className="p-8 flex items-center justify-between cursor-pointer hover:bg-white/[0.04]" onClick={() => toggleExpand(root.id)}>
                      ...
                      <div className="flex items-center gap-4">
                        <button onClick={(e) => {e.stopPropagation(); setEditingItem(root); setEditType('subject');}} className="p-3 text-blue-400"><Edit3 size={18}/></button>
                        <button onClick={(e) => {e.stopPropagation(); handleDelete('subjects', root.id);}} className="p-3 text-red-400"><Trash2 size={18}/></button>
                          ...
                          <div key={sub.id} className="ml-8 rounded-3xl border border-white/5 bg-white/[0.01]">
                            <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => toggleExpand(sub.id)}>
                              ...
                              <div className="flex gap-2">
                                <button onClick={(e) => {e.stopPropagation(); setEditingItem(sub); setEditType('subject');}} className="p-2 text-blue-400"><Edit3 size={14}/></button>
                                <button onClick={(e) => {e.stopPropagation(); handleDelete('subjects', sub.id);}} className="p-2 text-red-400"><Trash2 size={14}/></button>
                                ...
                                {chapters.filter(c => c.subject_key === sub.slug).map(chap => (
                                  <div key={chap.id} className="flex items-center justify-between bg-white/[0.02] p-4 rounded-2xl border border-white/5 group/chap hover:border-purple-500/30 transition-all">
                                    ...
                                    <div className="flex gap-1 opacity-0 group-hover/chap:opacity-100 transition-all">
                                      <button onClick={() => {setEditingItem(chap); setEditType('chapter');}} className="p-2 text-blue-400"><Edit3 size={14}/></button>
                                      <button onClick={() => handleDelete('chapters', chap.id)} className="p-2 text-red-400"><Trash2 size={14}/></button>
                                    </div>
                                  </div>
                                ))}
                              </div>
          ...
          {/* LIBRARY HUB VIEW - Restructured */}
          {view === 'library' && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-4">
              <div className="grid grid-cols-1 gap-3">
                {libraryItems.length === 0 ? (
                  <div className="p-10 text-center text-white/20 font-black uppercase text-sm tracking-widest">Library is empty for S{activeSemester}</div>
                ) : (
                  libraryItems.map(item => (
                    <div key={item.id} className="bg-white/[0.02] p-6 rounded-3xl border border-white/5 flex items-center justify-between group hover:border-blue-500/30 transition-all">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <h4 className="text-lg font-black text-white uppercase tracking-tighter">{item.label}</h4>
                          <div className="flex gap-3 mt-1">
                            ...
                            <span className="text-[9px] font-black uppercase px-2 py-1 bg-white/5 text-white/30 rounded border border-white/10">{item.subject_slug}</span>
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => {setEditingItem(item); setEditType('library');}} className="p-4 text-blue-400"><Edit3 size={18}/></button>
                        <button onClick={() => handleDelete('library', item.id)} className="p-4 text-red-400"><Trash2 size={18}/></button>
                      </div>
                    </div>
                  ))
                )}
          ...
          {view === 'questions' && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-6">
              <div className="flex items-center bg-white/5 rounded-2xl px-6 border border-white/10 max-w-xl mb-4">
                ...
                <input placeholder="Search questions..." className="flex-1 p-5 bg-transparent outline-none text-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-purple-500/50 transition-all" 
                    value={editingItem.title || editingItem.question_text || editingItem.label} 
                    onChange={e => setEditingItem({...editingItem, [editingItem.title ? 'title' : editingItem.question_text ? 'question_text' : 'label']: e.target.value})} />
                </div>
                {(editingItem.file_url !== undefined) && (
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/30 uppercase ml-4">Resource Link</label>
                    <input className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white font-mono text-xs outline-none focus:border-purple-500/50 transition-all" value={editingItem.file_url} onChange={e => setEditingItem({...editingItem, file_url: e.target.value})} />
                  </div>
                )}
              </div>
              <div className="flex gap-4 pt-10">
                <button onClick={handleSaveEdit} className="flex-1 py-5 bg-purple-600 hover:bg-purple-500 text-white rounded-[2rem] font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 shadow-lg shadow-purple-600/20 transition-all"><Save size={18}/> Push System Update</button>
      ...
      <div className="fixed bottom-10 right-10 flex items-center gap-6 px-8 py-4 bg-black/80 backdrop-blur-3xl border border-white/10 rounded-full z-50">
         ...
         <div className="text-white/60 text-[9px] font-black uppercase tracking-widest italic">Admin v6.5</div>
      </div>
    </div>
  );
}